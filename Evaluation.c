...
// Comme on va traiter les commandes en arriere plan, il faut écrire un handler pour le SIGCHLD
void sigchld_handler(int sig)
{
  int status;
  // Traiter les zombies
  while (waitpid(-1, &status, WNOHANG) > 0); //While car il peut etre plusieurs zombies en meme temps
}
int evaluateExpr(Expression *expr)
{
  ...
  if (shell_pid == 0)
    shell_pid = getpid();
  //On ajoute une nouvelle teste car on va appeler la fonction evaluate_expr récursivement donc il faut initialiser seulement pendant le première appel
  if (getpid() == shell_pid && first)
  {
  ...
  // Il faut qu'on masque le sigchld pour certains expressions
  sigset_t s;
  sigemptyset(&s);
  sigaddset(&s, SIGCHLD);
  // redirections
  if (expr->type == ET_REDIRECT)
  {
    ...
    if (pid > 0)
    {
      // On utilise wait donc il faut pas utiliser le handler
  ...
  // Il faut tester si la commande a gauche est execute sans probleme, si c'est le cas, il faut executer la commande a droite // Si la commande a gauche peut executer sans probleme, meme si la deuxieme cmd n'est pas valide, il faut executer la commande (d'après la simulation que j'ai fait sur // un shell)
  ...
  {
    int res;
    // On cree un fils pour traiter la commande avant &, mais le père ne l'attend pas
    pid_t pid;
    ...
    {
      // processus pere ne va pas attendre son fils, mais il faut traiter le signal sigchld avec le handler
      // On a déjà defini le handler, donc ici on fait un affichage pour informer l'utilisateur
      fflush(stdout);
      shellStatus = 0;
      return shellStatus;
    } // processus fils
